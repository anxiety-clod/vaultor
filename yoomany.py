...
try:
    import uvloop
    uvloop.install()
    print("✅ uvloop установлен - asyncio ускорен")
except ImportError:
    print("⚠ uvloop не установлен (для ускорения: pip install uvloop)")
...
app = Client(
    ...
    api_hash=API_HASH, 
    bot_token=BOT_TOKEN, 
    workers=100,  # Увеличено с 32 до 100
    max_concurrent_transmissions=50  # Лимит одновременных передач
)
...
async def get_db():
    ...
    global db_pool
    if db_pool is None:
        db_pool = await aiosqlite.connect(DB)
        ...
        await db_pool.execute("PRAGMA cache_size=10000")
        await db_pool.execute("PRAGMA temp_store=MEMORY")
    return db_pool

async def get_aiohttp_session():
    """Одна сессия для всех HTTP запросов"""
...
# ===== КЭШИРОВАНИЕ БАЛАНСА =====
async def get_balance_cached(uid):
    """Получение баланса с кэшированием"""
    global last_cache_cleanup
    
    current_time = time.time()
    
    # Очистка устаревшего кэша раз в 10 секунд
    if current_time - last_cache_cleanup > 10:
        balance_cache.clear()
        last_cache_cleanup = current_time
    
    # Проверяем кэш
    if uid in balance_cache:
        cache_time, balance = balance_cache[uid]
        if current_time - cache_time < 5:  # Кэш на 5 секунд
            return balance
    
    # Если нет в кэше или устарел - берем из БД
    balance = await get_balance(uid)
    balance_cache[uid] = (current_time, balance)
    return balance
...
async def get_user_profile(uid):
    ...
    """, (uid,))
    seller_deals = await cur.fetchone()
    seller_deals = seller_deals[0] if seller_deals else 0
    ...
    """, (uid,))
    buyer_deals = await cur.fetchone()
    buyer_deals = buyer_deals[0] if buyer_deals else 0
...
async def handle_text(client, message):
    ...
    try:
        ...
        # ======== Баланс ========
        if text == "💰 Баланс":
            bal = await get_balance_cached(uid)  # Используем кэшированную версию
            kb = main_keyboard_admin if uid == ADMIN_ID else main_keyboard
            await message.reply(
                f"💳 Ваш баланс: {bal} ₽\n\n"
    ...
    try:
        ...
        if data.startswith("check_payment_"):
            ...
            if label not in app.yoomoney_payments:
                await call.answer("❌ Платеж не найден или устарел", show_alert=True)
                return
                    ...
                    if resp.status == 200:
                        ...
                        if "operations" in data and data["operations"]:
                            ...
                            if op.get("status") == "success":
                                ...
                                # Проверяем сумму с погрешностью в 1 копейку
                                if abs(op_amount - expected_amount) < 0.01:
                                    # Зачисляем средства
                                    await change_balance(payment["user_id"], expected_amount)
                                    
                                    # Помечаем как проверенный
                                    payment["checked"] = True
                                    
                                    # Получаем новый баланс (используем кэш)
                                    new_balance = await get_balance_cached(payment["user_id"])
                                    ...
                                    await status_msg.edit_text(
                                        ...
                                        f"Сумма платежа не совпадает.\n"
                                        f"Ожидалось: {expected_amount} ₽\n"
                                        f"Получено: {op_amount} ₽\n\n"
                                ...
                                await status_msg.edit_text(
                                    ...
                                    "Платеж со статусом 'success' не обнаружен.\n"
                                    "Возможно, вы еще не оплатили или платеж еще обрабатывается.\n\n"
                                    "Попробуйте через минуту или обратитесь в поддержку.",
                                    parse_mode=ParseMode.HTML
                                )
                        else:
                            await status_msg.edit_text(
                                "❌ <b>Платеж не найден</b>\n\n"
                                "Операция с указанной меткой не обнаружена.\n"
                                "Убедитесь, что вы оплатили именно по этой ссылке.",
                                parse_mode=ParseMode.HTML
                            )
                    else:
                        await status_msg.edit_text(
        ...
        if data.startswith("pay_"):
            ...
            bal = await get_balance_cached(uid)  # Используем кэш
            if bal < amount:
        ...
        if data.startswith("confirm_"):
            ...
            seller_balance = await get_balance_cached(deal["seller_id"])
...
async def background_payment_checker():
    """Фоновая проверка платежей и очистка"""
    while True:
        try:
            await asyncio.sleep(30)  # Проверка каждые 30 секунд
            await cleanup_old_payments()
        except Exception as e:
            print(f"Ошибка в фоновом проверщике: {e}")

@app.on_shutdown()
async def shutdown():
    """Закрытие соединений при остановке"""
    global aiohttp_session, db_pool
    if aiohttp_session:
...
if __name__ == "__main__":
    ...
    print("🚀 Бот запущен (оптимизированная версия с профилем)")
    app.run()
