...
async def handle_text(client, message):
    ...
    try:
        ...
        if uid in user_states:
            ...
            # ======== Обработка суммы для пополнения по карте ========
...
async def callbacks(client, call):
    ...
    try:
        ...
        if data.startswith("rate_"):
            ...
            pool = await get_db()
            async with pool.acquire() as conn:
                row = await conn.fetchrow("SELECT seller_id, buyer_id, name FROM deals WHERE code=$1", code)
                
                if not row:
                    await call.answer("❌ Сделка не найдена", show_alert=True)
                    return
                    
                seller_id, buyer_id, name = row
        ...
        if data.startswith("pay_"):
            ...
            async with pool.acquire() as conn:
                ...
                buyer_id, seller_id, name, amount, status = row
        ...
        if data.startswith("confirm_"):
            ...
            pool = await get_db()
            async with pool.acquire() as conn:
                row = await conn.fetchrow("SELECT name FROM deals WHERE code=$1", code)
                name = row['name'] if row else "Сделка"
        ...
        if data.startswith("resolve_"):
            ...
            pool = await get_db()
            async with pool.acquire() as conn:
                row = await conn.fetchrow("SELECT buyer_id, seller_id, amount, name, status FROM deals WHERE code=$1", code)
                
                if not row:
                    await call.answer("❌ Сделка не найдена", show_alert=True)
                    return
                    
                buyer_id, seller_id, amount, name, status = row
...
async def cleanup_old_payments():
    ...
    for request_id, request in app.replenish_requests.items():
        if current_time - request.get("timestamp", 0) > 86400:
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("✅ База данных: PostgreSQL (Supabase)")
    print("✅ Пополнение: только карта РФ")
