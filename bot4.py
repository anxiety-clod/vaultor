...
app = Client(
    ...
    api_hash=API_HASH, 
    bot_token=BOT_TOKEN, 
    workers=100,  # Увеличено для скорости
    max_concurrent_transmissions=50  # Одновременная отправка
)
...
async def get_db():
    ...
    if db_connection is None:
        ...
        await db_connection.execute("PRAGMA cache_size=-20000")  # 20MB кэш
...
async def callbacks(client, call):
    ...
    try:
        ...
        if data.startswith("check_payment_"):
            ...
            # Убираем лишний call.answer() - он уже есть выше
            return

        # ===== ИСПРАВЛЕННАЯ БЫСТРАЯ РАССЫЛКА =====
        if data == "confirm_broadcast":
            ...
            # Отправляем ответ на callback ДО начала рассылки (чтобы не истек)
            await call.answer("✅ Рассылка начата", show_alert=False)
            
            # Максимально быстрая рассылка через asyncio.gather
            tasks = []
            ...
            # Отправляем результат в новом сообщении (не редактируем старое)
            await app.send_message(
                uid,
        ...
        # Если дошли сюда без return - просто отвечаем
        await call.answer()
...
if __name__ == "__main__":
    ...
    print("🚀 Бот запущен (максимальная скорость, без uvloop)")
    app.run()
